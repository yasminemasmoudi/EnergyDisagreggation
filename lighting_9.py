...
def main():
    ...
    y_train = df1_train['lighting_9'].values
    X_val = df1_val[['mains_1','mains_2']].values
    y_val = df1_val['lighting_9'].values
    X_test = df1_test[['mains_1','mains_2']].values
    y_test = df1_test['lighting_9'].values
    print(X_train.shape, y_train.shape, X_val.shape, y_val.shape, X_test.shape, y_test.shape)
    
    # OVEN 3 TRAINING
    model = build_fc_model()
    hist = train_model("models/lighting_9_h1_2.hdf5",model,X_train,y_train)
    ...
    model = load_model('models/lighting_9_h1_2.hdf5', custom_objects={"f1_m": f1_m, "precision_m": precision_m, "recall_m": recall_m})
    pred = model.predict(X_test).reshape(-1)

    # mse_loss = mse_loss(pred, y_test)
    # mae_loss = mae_loss(pred, y_test)
    
    # mlflow.log_metric("mse_loss", mse_loss)
    # mlflow.log_metric("mae_loss", mae_loss)

    mlflow.sklearn.log_model(model, "lighting_9_model")

    train_loss = hist.history['loss']
    val_loss = hist.history['val_loss']

    plot_each_app(df1_test, dates[1][17:], pred, y_test, 
                'FC model: real and predict lighting 9 on 6 test day of house 1', look_back = 50)
