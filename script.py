...
with mlflow.start_run():
    ...
    model_1.compile(loss='mean_squared_error', optimizer=adam, metrics=['accuracy', f1_m, precision_m, recall_m])
    start = time.time()

    checkpointer = ModelCheckpoint(filepath="models/oven_3_h1_2.hdf5", verbose=0, save_best_only=True)

    hist_1 = model_1.fit(X_train1, y_train1, batch_size=512, verbose=1, epochs=epochs, validation_split=0.33, callbacks=[checkpointer])

    loss, accuracy, f1_score, precision, recall = model_1.evaluate(X_test1, y_test1, verbose=0)
    ...
    model_1 = load_model('models/oven_3_h1_2.hdf5', custom_objects={"f1_m": f1_m, "precision_m": precision_m, "recall_m": recall_m})
    pred_1 = model_1.predict(X_test1).reshape(-1)

    mse_loss_1 = mse_loss(pred_1, y_test1)
    mae_loss_1 = mae_loss(pred_1, y_test1)

    mlflow.log_metric("mse_loss_1", mse_loss_1)
    mlflow.log_metric("mae_loss_1", mae_loss_1)

    mlflow.log_artifact(local_path="models/oven_3_h1_2.hdf5", artifact_path="models_pickle")
...
with mlflow.start_run():
    ...
    model_2.compile(loss='mean_squared_error', optimizer=adam, metrics=['accuracy', f1_m, precision_m, recall_m])
    start = time.time()

    checkpointer = ModelCheckpoint(filepath="models/refrigerator_5_h1_2.hdf5", verbose=0, save_best_only=True)

    hist_2 = model_2.fit(X_train2, y_train2, batch_size=512, verbose=1, epochs=epochs, validation_split=0.33, callbacks=[checkpointer])

    loss, accuracy, f1_score, precision, recall = model_2.evaluate(X_test2, y_test2, verbose=0)
    print('Finish training. Time: ', time.time() - start)
    ...
    model_2 = load_model('models/refrigerator_5_h1_2.hdf5', custom_objects={"f1_m": f1_m, "precision_m": precision_m, "recall_m": recall_m})
    pred_2 = model_2.predict(X_test2).reshape(-1)

    mse_loss_2 = mse_loss(pred_2, y_test2)
    mae_loss_2 = mae_loss(pred_2, y_test2)

    mlflow.log_metric("mse_loss_2", mse_loss_2)
    mlflow.log_metric("mae_loss_2", mae_loss_2)

    mlflow.log_artifact(local_path="models/refrigerator_5_h1_2.hdf5", artifact_path="models_pickle")
...
plot_each_app(df1_test, dates[1][17:], pred_2, y_test2, 
...
with mlflow.start_run():
    ...
    model_4 = load_model('models/dishwaser_6_h1_2.hdf5', custom_objects={"f1_m": f1_m, "precision_m": precision_m, "recall_m": recall_m})
    pred_4 = model_4.predict(X_test4).reshape(-1)

    mse_loss_4 = mse_loss(pred_4, y_test4)
    mae_loss_4 = mae_loss(pred_4, y_test4)
